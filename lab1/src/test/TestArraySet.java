...
public class TestArraySet {
	...
	public final void testAddAll() {
		int nbrsToAdd = 1000;
		SimpleSet<Integer> inputSet = new ArraySet<Integer>();
		
		for(int i = 0; i < nbrsToAdd; i++) {
			inputSet.add(i);
		}
		
		assertEquals("List didn't change" , true, s.addAll(inputSet)); // test insertion
		assertEquals("List changed" , false, s.addAll(inputSet)); // test duplicate insert
		assertEquals("Wrong size, all elements didnt get addedededed!", nbrsToAdd, s.size()); // correct amount of elements?		
	}
}
